...
var cryptico = (function() {
    ...
    function CodeAndEncode(_key,_str)
    {
        ...
        for( i = 0; i<_str.length; i++ )
        {
            ...
            var code = String.fromCharCode( _strXOR );          //fromCharCode() 可接受一个指定的 Unicode 值，然后返回一个字符串。
            if (codedStr==null){
                codedStr = code;
            }
            else{
    ...
    my.rsa_enc = function(pub_key,r,s){
        setMaxDigits(131);  
        var key = new RSAKeyPair("10001", '', pub_key); 
        //            alert("salt is:"+s);
        //            alert("plain random is:"+r);
        var PlainText = CodeAndEncode(s,r);
        //PlainText = CryptoJS.enc.Utf8.parse(PlainText);
        PlainText = cryptico.utf16to8(PlainText);
        //PlainText = "";
        //var b64 = cryptico.encode64(PlainText);
        //alert("b64 xor result is:"+b64);
        var enc1 = encryptedString(key, PlainText);
        enc1 = cryptico.encode64(enc1);
        return enc1;

    }

    my.aes_enc = function(text,key){
        var text = cryptico.utf16to8(text);
        var b64 = cryptico.encode64(text);
        var key_hash = CryptoJS.MD5(key); 
        var key = CryptoJS.enc.Utf8.parse(key_hash);
        var iv = CryptoJS.enc.Utf8.parse('1234567812345678');
        var password_enc = CryptoJS.AES.encrypt(b64, key, {
            iv: iv,
